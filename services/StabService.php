...
class StabService {
    ...
    public function stabDb($numberOfLoopIterations) {
        ...
        $stmt = $this->_db->query("SELECT MAX(user_id) as max_id FROM users");
        ...
        for ($i = $j; $i < $j + $numberOfLoopIterations; $i++) {
            try {
                ...
                if (!$this->_db->exec($sql)) {
                    echo $sql;
                    $this->errors[] = "Пользователь с id = $i не  создан";
                    continue;
                }
...
<br />" . $texts[$random1] . " '";
                ...
                if (!$this->_db->exec($sql)) {
                    echo $sql;
                    $this->errors[] = "Дополнительная информация для поста с id = $postId не создана";
                    continue;
                }
                ...
                if (!empty($tags)) {
                    foreach ($tags as $tag) {
                        ...
                        if (!$this->_db->exec($sql)) {
                            echo $sql;
                            $this->errors[] = "Тэг $tag к посту № $postId не создан";
                            continue;
                        }
                    }
                }
    ...
    public function isNounForTag($text){
        $symbol1 = 'а';
        $symbol2 = 'ь';
        $tags = [];
